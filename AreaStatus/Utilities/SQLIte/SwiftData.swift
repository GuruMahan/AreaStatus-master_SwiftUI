...
public struct SwiftData {
        ...
        let task: ()->Void = {
            ...
            for sqlStr in sqlArr {
                if let err = SQLiteDB.sharedInstance.executeChange(sqlStr) {
                    SQLiteDB.sharedInstance.close()
                    if sqlArr.firstIndex(of: sqlStr) != nil {  //find(sqlArr, sqlStr)
                        // print("Error occurred on array item: \(index) -> \"\(sqlStr)\"")
                    }
                    //tempBundleList.indexOf(themeBundleEntity!)
                    error = err
                    return
                }
            }
            //find is unavailable: call the 'indexOf()' method on the collection
            //close database connection
            SQLiteDB.sharedInstance.close()
    ...
    public static func saveUIImage(_ image: UIImage) -> String? {
        ...
        if !FileManager.default.fileExists(atPath: imageDirPath) {
            do{
                try FileManager.default.createDirectory(atPath: imageDirPath, withIntermediateDirectories: false, attributes: [:])
            } catch {
                //print("Error creating SwiftData image folder")
                return nil
            }
        }
        //cannot invoke 'createDirectoryPath'
    ...
    fileprivate class SQLiteDB {
        ...
        class func createPath() -> String {
            ...
            let docsPath = NSSearchPathForDirectoriesInDomains(FileManager.SearchPathDirectory.documentDirectory, FileManager.SearchPathDomainMask.userDomainMask, true)[0] as String
            let databaseStr = "AreaStatus.sqlite"
            ...
            case "CHARACTER(20)", "VARCHAR(255)", "VARYING CHARACTER(255)", "NCHAR(55)", "NATIVE CHARACTER", "NVARCHAR(100)", "TEXT", "CLOB":
             //   let text = UnsafePointer<Int8>(sqlite3_column_text(statement, index))
              //  return String(cString: text!) as AnyObject?
                let text = UnsafePointer(sqlite3_column_text(statement, index))
                return String(cString: text!) as AnyObject?
            case "BLOB", "NONE":
...
//                dateFormatter.dateFormat = GlobalReusability.DateFormats.LocalDBDateTimeFormat
                // let text
                _ = UnsafePointer(sqlite3_column_text(statement, index))
            ...
            while next {
                ...
                if status == SQLITE_ROW {
                    ...
                    for i: Int32 in 0 ..< columnCount {
                        ...
                        if let columnType = ((sqlite3_column_decltype(pStmt, i)) as? String)?.uppercased() {
...
protocol SqliteExecution{
    
}
extension SqliteExecution{
    func executeChange(_ sQuery:String){
        if let err =  SD.executeChange(sQuery){
            print(err)
        }
    }
    
    func executeQuery(_ sQuery:String)->[SD.SDRow]{
        let (result,_) = SD.executeQuery(sQuery)
